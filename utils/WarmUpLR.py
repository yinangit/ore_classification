...
import torch
class Two_stage_WarmupLR(_LRScheduler):
    def __init__(self, optimizer, init_lr=1e-3, num_warmup=5,num_warmup2=5,t1=10,t2=20, warmup_strategy='linear'):
        if warmup_strategy not in ['linear', 'cos', 'constant']:
            raise ValueError("Expect warmup_strategy to be one of ['linear', 'cos', 'constant'] but got {}".format(warmup_strategy))
        self._scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=t1) # 二者共享同一内存的optimizer !!!
        # self._scheduler2 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=t2)
        self._scheduler2 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=t2-t1) # 函数所关注的epoch是相对值，而不是绝对值（step一次，才算一次）
        self._init_lr = init_lr
        self._num_warmup = num_warmup
        self._num_warmup2 = num_warmup2
        self._t1 = t1
        self._t2 = t2
        self._step_count = 1
        # Define the strategy to warm up learning rate 
    ...
    def _format_param(self):
        ...
        for group in self._scheduler.optimizer.param_groups:
            group['warmup_max_lr'] = group['lr']
            group['warmup_initial_lr'] = min(self._init_lr, group['lr'])
        for group in self._scheduler2.optimizer.param_groups:
            group['warmup_max_lr'] = group['lr']
            group['warmup_initial_lr'] = min(self._init_lr, group['lr'])
        ...
        if step_num <= self._num_warmup:
            for group in self._scheduler.optimizer.param_groups:
                computed_lr = self._warmup_func(group['warmup_initial_lr'], 
                                                ...
                                                step_num/self._num_warmup)
                lrs.append(computed_lr)
        elif self._num_warmup < step_num <= self._t1:
            lrs = self._scheduler.get_lr()
            self._t1_lr = lrs[0]
        elif self._t1 < step_num <= self._t1 + self._num_warmup2:
            for group in self._scheduler2.optimizer.param_groups:
                computed_lr = self._warmup_func(self._t1_lr, 
                                                group['warmup_max_lr'],
                                                (step_num-self._t1)/self._num_warmup)
                lrs.append(computed_lr)
        else:
            lrs = self._scheduler2.get_lr()
        return lrs
    ...
    def step(self, *args):
        if self._step_count <= self._num_warmup:
            ...
            for param_group, lr in zip(self._scheduler.optimizer.param_groups, values):
                param_group['lr'] = lr
            self._step_count += 1 
        elif self._num_warmup < self._step_count <= self._t1:
            self._scheduler.step(*args)
            self._step_count += 1
        elif self._t1 < self._step_count <= self._t1 + self._num_warmup2:
            values = self.get_lr()
            for param_group, lr in zip(self._scheduler2.optimizer.param_groups, values):
                param_group['lr'] = lr
            self._step_count += 1
        else:
            self._scheduler2.step(*args)
            self._step_count += 1